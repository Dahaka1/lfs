...
def mock_programs(monkeypatch):
    async def get_default_programs(*args, **kwargs):
        return tests.additional.stations.generate_station_programs(amount=12, as_schema=True)

    monkeypatch.setattr(StationProgram, "get_default_programs", get_default_programs)
